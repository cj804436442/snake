...
function Snake(){
	...
	this.directonNum = {//存储蛇走的方向，用一个对象来表示
		left:{
			x:-1,
			y:0,
...
Snake.prototype.init = function(){
	...
	//给蛇添加一条属性，用来表示蛇走的方向
	this.direction = this.directonNum.right;//默认让蛇往右走
}
...
Snake.prototype.getNextPos = function(){
	...
	this.pos.forEach(function(value){
		if(value[0]==nextPos[0] && value[1]==nextPos[1]){
			//如果数组中的两个数据都相等，就说明下一个点在蛇身体里面能找到，代表撞到自己
			selfCollied = true;
		}
	})
	if(selfCollied){
...
Snake.prototype.strategis = {
	move:function(format){  //这个参数用于决定要不要删除最后一个方块(蛇尾)，当传了这个参数后就表示要做的事情是吃
		...
		if(!format){   //若format的值为false，表示需要删除(出了吃之外的操作)
			this.tail.remove();
	...
	eat:function(){
		this.strategis.move.call(this,true);
		createFood();
		game.score++;
	},
	die:function(){
...
Game.prototype.init = function(){
	...
	document.onkeydown = function(ev){
		...
		}else if(ev.which==39 && snake.direction != snake.directonNum.left){
			snake.direction = snake.directonNum.right;
		}else if(ev.which==40 && snake.direction != snake.directonNum.up){
			snake.direction = snake.directonNum.down;
		}
	}
